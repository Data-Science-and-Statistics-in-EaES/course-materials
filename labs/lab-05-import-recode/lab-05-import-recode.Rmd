...
2.  Identify the three partial datasets within the starter file `data` folder of your your project directory (`earthquakes-1 to earthquakes-3`). Insert a code chunk under Exercise 2 in your starter file. Import the three files into RStudio using three lines of code and an appropriate read function for each file. We give you some partial code below to get started on the first one.

```{r label-me1, eval=FALSE}
earthquakes_1 <- read_csv("data/___")
```

3.  Write a labeled code chunk to view the imported data frames under Exercise 3 in your starter file, and comment with text narrative on: **1)** what is missing from each data frame when compared to the `openintro::earthquakes` version; **2)** what is strange about the `deaths` variable in the object you created when importing `earthquakes-3.xlsx`; and **3)** why will the code chunk below (labeled `coerce-earthquakes-3`) fix the problem with `deaths`?

**After you respond to these three prompts, make sure you copy the code chunk into your starter file under your responses to exercise 3.** Notice that the syntax for `col_types` within `read_xlsx`, is slightly different to that for `read_csv` or `read_txt`.
Always check the function help page if you're not sure!
...
4.  Insert a labeled code chunk which uses a pipeline of join functions to merge the data from the three partial datasets. Don't assign the output to anything, but pipe the joined data into `skim()` to confirm the output roughly matches that of the `inspect-earthquakes` chunk at the start of this lab.
...
6.  Complete and add to the code below such that `earthquakes-badnames-na.csv` is read into RStudio **and** any bad values ("." and "NA") are automatically assigned to `NA`.
...
9.  Copy your code from exercise 8 into the next exercise 9 labeled `revlevel-month`. Add to the mutate function so that the default alphabetical ordering of the `month` variable is re-leveled to match the natural ordering of the months (January through December). Assign the new output to `earthquakes_correct`. Add a second, short pipeline that takes `earthquakes_correct` as input and applies the `count()` function to confirm that the `month` factor levels are ordered correctly.

```{r relevel-month, message = F, warning = F}

```

10. Write a new short a pipeline that writes `earthquakes_correct` as an `.rds` file. In a separate pipeline, use `readRDS` to read the file back in and, using count() again, confirm that the ordering of `month` is retained in an RDS file.
