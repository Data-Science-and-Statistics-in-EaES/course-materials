...
For this lab, we will be using the data we generated earlier this semester the class survey.
...
1.  Look in the Lab 6 `/data` sub-directory and identify the name and file type of the dataset we need to load. Then complete the code chunk below in your starter file to load the class survey data and assign the imported data to a new object called `class_survey`. State below your code chunk how many observations there are in the dataset and what the data type of each variable is.
...
2.  Copy the code chunk below into Exercise 2 in your starter file and complete the pipline which will turn this dataset into a "long" format. Remember to change `eval` to `T` when you are ready to run the chunk.
...
5.  State in text narrative what `input` each function requires and why this input is equivalen to a vector. Then copy your code from Exercise 4 into the code chunk provided under Exercise 5 in the starter file, and change the input to a more generic placeholder: `response_vector`. Again, you can leave `eval = F` for this code chunk because we don't need to run it.
...
8.  Insert a code chunk under Exerise 8 in your starter file.
...
9.  Uh oh!
    ...
    State in text narrative below Exercise 9 in your starter file why we only get one output?
