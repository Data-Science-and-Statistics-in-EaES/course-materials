...
monroe_fluxes %>% 
  ...
  select(date, fco2, season, winter) %>%
  print(n = 5)
```
...
linear_reg() %>%
  set_engine("lm") %>%
  fit(fco2 ~ factor(winter), data = monroe_fluxes) %>%
  tidy()
```
...
.large[
$$\widehat{fco2} = -5.223 - 3.562 \times winter$$
]
...
- **Fal ** CO2 fluxes are expected, on average, to be 0.8394269 nmol m-2 s-1.
- **Spring** CO2 fluxes are expected, on average, to be 3.0774139 nmol m-2 s-1 more negative than in Fall.
- **Summer** CO2 fluxes are expected, on average, to be 6.0625599 nmol m-2 s-1 more negative than Fall.
- **Winter** CO2 fluxes are expected, on average, to be 0.2359090 nmol m-2 s-1 more positive than Fall.
