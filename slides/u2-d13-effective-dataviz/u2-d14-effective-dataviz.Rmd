...
.pull-left-narrow[
```{r pie-3d, echo = FALSE, out.width="60%"}
...
```{r ref.label="default-opinion", echo = FALSE, out.width= "40%"}
```
]
...
.panel[.panel-name[Plot]
```{r ref.label="infreq", echo = FALSE, out.width= "40%"}
```
]
...
.panel[.panel-name[Plot]
```{r ref.label="region-default", echo = FALSE, out.width="40%"}
```
]
