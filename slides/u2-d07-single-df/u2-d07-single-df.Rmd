...
### A data table (the unit of analysis) where each row is an observation and each column is a variable.
...
### The manipulation of a data table into a tidy form that can be easily visualized and analyzed, using verb functions.

---

class: middle

## What package do we use (primarily) for data wrangling?
...
## How do you load the wrangling package?
...
## What are the four rules of wrangling functions?
...
### 1. 1st argument is data
### 2. following arguments detail the action to be performed 
### 3. input is always a data frame
### 4. function does not alter original data frame (unless you assign it)

---

class: middle

## What is the difference between piping and layering?
...
### Piping (%>%) is sends the output from one line/function to the input of the next
### e.g.  data_frame **%>%** select(...) **%>%** filter() **%>%** ggplot()...

### Layering (+) is adding graphical layers to plots in ggplot()
### e.g.  ggplot(data = ..., aes(x = x, y = y)) **+** geom_point() **+** facet_wrap(~...)

---

class: middle

## What are the four functions we've covered so far?
...
### `select()` - pick/drop columns/variables by name
### `slice()` - pick rows using index(es)
### `arrange()` - reorder rows using variable values
### `filter()` - pick rows matching certain criteria

---

## Handouts!

### The handout covers some wrangling functions and logical operators 
### You already have a handout for `ggplot()` (or ask me for the pdf!)
...
catch_data %>%
  summarise(
    min_catch = min(values, na.rm = TRUE),
    mean_catch = mean(values, na.rm = TRUE),
    median_catch = median(values, na.rm = TRUE),
    max_catch = max(values, na.rm = TRUE)
    )
```
