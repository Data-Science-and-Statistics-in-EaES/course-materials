...
```{r histogram-2, echo = F, out.width="50%"}
monroe_fluxes %>% 
  ggplot(aes(fco2)) +
  geom_histogram() + 
  labs(x = expression("Net CO"[2]*" Flux (nmol m"^{-2}*" s"^{-1}*")")) +
...
```{r co2-flux-plot-no-se-2, warning=FALSE}
monroe_fluxes %>% 
  filter(date > "2020-07-01" & date < "2020-07-05") %>% 
  ggplot(aes(x = sw_in, y = fco2)) +
  geom_point() +
...
```{r co2-flux-plot-cosmetics-2, warning=FALSE}
monroe_fluxes %>% 
  filter(date > "2020-07-01" & date < "2020-07-05") %>% 
  ggplot(aes(x = sw_in, y = fco2)) +
  geom_point() +
...
```{r co2-flux-plot-gam-2,  warning=FALSE}
monroe_fluxes %>% 
  filter(date > "2020-07-01" & date < "2020-07-05") %>% 
  ggplot(aes(x = sw_in, y = fco2)) +
  geom_point() +
...
```{r co2-flux-plot-loess-2, fig.show="hide", warning=FALSE}
monroe_fluxes %>% 
  filter(date > "2020-07-01" & date < "2020-07-05") %>% 
  ggplot(aes(x = sw_in, y = fco2)) +
  geom_point() +
...
```{r co2-flux-plot-residuals-2, fig.show="hide", warning=FALSE, echo = F}
co2_flux_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(fco2 ~ sw_in, data = filter(monroe_fluxes, date > "2020-07-01" & date < "2020-07-05"))
...
.pull-left-narrow[
.question[
How might the relationship between sunlight and CO2 flux be affected by the season of the year?
]
]
.pull-right-wide[
```{r ref.label = "co2-flux-seasonality", echo = FALSE, warning = FALSE, out.width = "40%"}
```
]
]
...
.panelset[
.panel[.panel-name[Plot]
```{r ref.label = "extrapolation", echo = FALSE, warning = FALSE, out.width = "40%"}
```
]
.panel[.panel-name[Code]
