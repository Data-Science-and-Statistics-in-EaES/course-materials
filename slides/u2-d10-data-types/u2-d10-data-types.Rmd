...
```{r}
cat_lovers %>%
  summarise(median_cats = median(number_of_cats))
```
...
```{r echo=FALSE, caption="Help for mean", out.width="60%"}
knitr::include_graphics("img/median-help.png")
```
...
```{r}
cat_lovers %>%
  summarise(median_cats = mean(number_of_cats, na.rm = TRUE))
```
...
## Now, let's learn about data types in R!
...
--
- **Implicit coercion** happens when you use a vector (or dataframe column) in a specific context that expects a certain type of vector

---

```{r echo=FALSE, caption="Data type coerceion", out.width="75%"}
...
- RStudio Cloud > `Lab 04 - Type coercion` > open `type-coercion.Rmd` and knit.
