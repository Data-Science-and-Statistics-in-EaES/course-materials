---
title: "EAES 494 Practice Midterm Instructions"
output: 
  tufte::tufte_html:
    css: ../exam.css
...
1.  **(5 points)** Under Ex. 1 in your starter file, write and label a code chunk that uses `glimpse()` to allow you to inspect the `earthquakes` dataset. After your code chunk, state **(1)** how many rows and variables there are in the `earthquakes` dataset and **(2)**, of the variables, how many are categorical and how many are numerical.

In a tidy data frame, each column is a variable, and each row is an observation.

2.  **(5 points)** Under Ex. 2 in your starter file, write and label a code chunk that uses `names()` to allow you to inspect the variable names in the `earthquakes` dataset. After your code chunk, state two stylistic choices for these variable names that means they accord with good R syntax.
...
-   Under Ex. 3 in your starter file, write and label a code chunk that uses the `count()` function to output the number of observations (rows) for each `region` in the `earthquakes` database. The output should be arranged in descending order by the number (`n`) of observations.
...
-   Under Ex. 4 in your starter file, copy and complete the code chunk provided below.
...
-   Under Ex. 5 in your starter file, copy and complete the code chunk provided below that will generate histograms of earthquake `richter`.
...
-   Under Ex. 6 in your starter file, write and label a code chunk that takes `earthquakes` as input and uses a pipeline of wrangling functions to:
...
-   Under Ex. 7 in your starter file, state why `April` appears on the left, instead of `January`.
...
earthquakes_subset %>% 
  mutate(___ = fct_relevel(___,
    ...
    geom_violin() +
    theme_classic()
write_csv(earthquakes, "earthquakes-issues.csv")
```
...
-   Write and label a code chunk that will read the problematic dataset from your data folder, into R, and assign the data to the object `earthquakes_with_issues`.
