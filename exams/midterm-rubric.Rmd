...
The center (average) body mass does appear to depend on species because the center (average) of the Gentoo distribution is farther right / higher / larger.

### Exericse 6

```{r subset-flipper-data}
penguins_flippers <- penguins %>% 
...
```{r visualize-penguin-flipper-length}
penguins_flippers %>% 
...
The factor levels are assigned to labels by default based on alphabetical order (1 = female, then 2 = male).
...
The variable names have spaces, and the sex has a bad value "\#" and year has a bad entry "."
