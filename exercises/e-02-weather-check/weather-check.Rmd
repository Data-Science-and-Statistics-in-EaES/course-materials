...
-   Notice that if the lats option "A specific website or app" is selected, then the respondent must provide a written answer which gets saved in `weather_source_site`
...
There are `___` combinations of responses here (`length(weather_check_daily$weather_source)` x `length(weather_check_daily$hhold_income)`) so it's hard to see which combinations are most frequent.

We can arrange the responses from most frequent to least frequent using: (note the first three lines are the same)
...
First, let's assign the frequency data we need as a new object:

```{r}
weather_check_counts <- weather_check_daily %>%
  group_by(female, weather_source) %>%
  summarise(weather_source_count = n())
```

Next, let's pass the new data object to a the `ggplot` plotting function:
...
```{r warning = FALSE}
ggplot(data = weather_check_counts, 
       mapping = aes(x = weather_source, y = weather_source_count, color = age)) +
  geom_boxplot() +
